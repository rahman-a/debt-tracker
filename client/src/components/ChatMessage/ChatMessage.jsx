...
const Message = ({
  ...
  const parseFileObject = (_) => {
    ...
    const CORRUPTED =
      message.type === 'file' ? t('document-corrupted') : t('image-corrupted')
  ...
  const textMessage = <p> {message.content} </p>

  const errorMessage = <p style={errorParagraphStyle()}> {error} </p>
  ...
  const fileMessage = (
    <figure style={{ width: '10rem' }}>
      <img
        src={`/images/placeholder/${fileType}`}
        alt='files'
        style={{ width: '100%', objectFit: 'cover' }}
      />
      <p style={{ overflow: 'hidden' }}> {fileName} </p>
  ...
  return (
    ...
    >
      {!error && (
        <p style={{ fontSize: '1rem', fontWeight: '600', color: '#1a374d' }}>
          {lang === 'ar'
            ? message.sender.fullNameInArabic
            : message.sender.fullNameInEnglish}
        </p>
      )}
      {error
        ? errorMessage
      ...
      {!error && (
        <p style={{ color: '#8f8f8f' }}>
          ...
          <span style={{ color: isMessageDelivered && 'green' }}>
            {isMessageSent || isMessageDelivered ? <DoubleCheck /> : <Check />}
          </span>
        </p>
      )}
    </div>
