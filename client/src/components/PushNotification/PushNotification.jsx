...
const PushNotification = ({ idx, data }) => {
  ...
  const defineBottomPosition = (_) => {
    const containerHeight =
      notificationRef.current.getBoundingClientRect().height
    setBottomPosition(idx * containerHeight + spaceBetweenNotifications + 'px')
    spaceBetweenNotifications = idx >= 1 ? spaceBetweenNotifications + 10 : 60
  }
  ...
  return (
    <div
      className={style.pushNotification}
      style={{
        right: toggleNotification,
        bottom: idx > 0 ? bottomPosition : '5rem',
      }}
      onClick={() => setToggleNotification('-50rem')}
      ref={notificationRef}
    >
