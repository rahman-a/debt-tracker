...
const Chat = ({ socket, setUnSeenMessage }) => {
  ...
  const { conversations: chat } = useSelector(
  ...
  const checkConversationIsActive = (message) => {
    ...
    const isFound = copiedConversations.find(
  ...
  const loadMessageOnScrollHandler = (e) => {
    // console.log({ scroll: e.target.scrollTop })
  }

  useEffect(() => {
    const scrollElement = scrollElementRef.current
    scrollElement.addEventListener('scroll', loadMessageOnScrollHandler)
    return () =>
      scrollElement.removeEventListener('scroll', loadMessageOnScrollHandler)
  }, [scrollElementRef])
