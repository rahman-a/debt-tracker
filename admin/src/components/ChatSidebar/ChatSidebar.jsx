...
import { Loader, ChatRoomCreation } from '../../components'
...
const Sidebar = ({ socket, unSeenMessage, setUnSeenMessage }) => {
  ...
  const { loading: chat_loading, conversation: chat } = useSelector(
  ...
  const conversationsMembers = (members) => {
    if (members) {
      const member = members.find((member) => member._id !== staff._id)
      if (member) {
        return {
          _id: member._id,
          name:
  ...
  const markPeerMessageAsRead = (_) => {
    dispatch(
      actions.chat.markPeerMessagesAsReceived({
        conversation: id,
        sender: peerId,
      })
    )
  ...
  return (
    <div className={`${style.sidebar} ${chat ? style.sidebar__off : ''}`}>
      {chat_loading && (
        <div className={style.sidebar__overlay}>
      ...
      <div
        className={`${style.sidebar__search} ${
          lang === 'ar' ? style.sidebar__search_ar : ''
        }`}
      >
        <div>
          {isSearch && (
            ...
            conversations.map((conversation) => (
              <div
                className={`${style.sidebar__body_chat} 
                        ...
                        ${
                          unSeenMessage === conversation._id
                            ? style.sidebar__body_chat_unseen
                            : ''
                        }`}
                style={{
                  backgroundColor:
                    selected === conversation._id ? '#1a374d1a' : 'unset',
                }}
              ...
              >
                {conversation.isRoom && (
                  <span
                    onClick={(e) => deleteRoomHandler(e, conversation._id)}
                    className={style.sidebar__body_chat_delete}
                    style={{
                      right: lang === 'ar' ? 'unset' : '0.3rem',
                      left: lang === 'ar' ? '0.3rem' : 'unset',
                    }}
                  >
                    <Trash />
                  </span>
                ...
                <figure>
                  ...
                  {!conversation.isRoom &&
                    onlineUsers.includes(
                      conversationsMembers(conversation.members)?._id
                  ...
                  <img
                    src={
                      conversation?.image
                        ...
                        : `/api/files/${
                            conversationsMembers(conversation.members).image
                          }`
                    }
                    alt='chat'
                  />
                ...
                <div className={style.sidebar__body_overview}>
                  ...
                  {conversation.lastMessage && (
                    <p>{cutLongText(conversation.lastMessage)}</p>
                  )}
                </div>
              </div>
            ))
