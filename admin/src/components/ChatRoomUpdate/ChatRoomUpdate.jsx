...
const Room = ({isRoomUpdate, setIsRoomUpdate, chatData}) => {
    ...
    const saveRoomHandler = _ => {
       ...
       if(img && (img.size / 1024) > 500 ) {
        ...
        img && data.append('image', img)
        
        dispatch(actions.chat.updateRoom(chatData.conversation, data))
    }

    const clearAlert = () => {
        dispatch({type:constants.chat.UPDATE_ROOM_RESET})
        setErrors(null)
    }
    ...
    useEffect(() => {
    if(chatData) {
        setSelectedMembers(chatData.members)
    ...
    return (
        ...
        />
            ...
            <Modal show={isRoomUpdate} onHide={() => setIsRoomUpdate(false)}>
                <Modal.Header>
                    <p>
                        <span style={{
                            marginRight:'1rem', 
