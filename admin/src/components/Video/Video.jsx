...
const Video = () => {
  ...
  const updateContentHandler = (e) => {
    ...
    const video = data.video ? data.video : content.video
    dispatch(
      actions.content.updateContent(type, content._id, {
        header,
        body,
        type,
        video,
    ...
    <>
      ...
      <div className={style.contact}>
        <Form>
          ...
          <Form.Group controlId='formBasicHeader' className='mb-3'>
            <Form.Label>{t('video-id')}</Form.Label>
            <Form.Control
              size='lg'
              row={5}
              name='video'
              placeholder={content ? content.video : 'Enter Video Id'}
              onChange={(e) => handleInput(e)}
            />
          </Form.Group>
