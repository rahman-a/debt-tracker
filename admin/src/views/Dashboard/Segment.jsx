...
const Segment = ({ value, title, icon, type, page }) => {
  ...
  const dashboardSegmentClasses = classnames(style.dashboard__segment_info, {
    [style.dashboard__segment_info_ar]: lang === 'ar',
  })
  ...
  return (
    <div
      className={style.dashboard__segment}
      style={{ color: mainColor[type] }}
      onClick={() => navigate(page)}
    >
      <span>{icon}</span>
      <div className={dashboardSegmentClasses}>
        <h3>{t(title)}</h3>
        <p>
